...
description: Spec-anchored - a spec é auditada mecanicamente contra o código e continua verdadeira. Fluxo Especificar, Projetar, Tarefas, Plano, Executar, Auditar, Aprender. Rastreabilidade história→critério de aceite→tarefa→teste, pronto executável (cada critério de aceite vira teste), suposições e perguntas como cidadãs, princípios verificáveis (LGPD/educação), lições com lastro mecânico (motor recusa lição sem sinal de audit/verify) e plano com paralelismo opcional - SEMPRE pergunta quais tarefas paralelizar (worktrees + headless, ou sequencial), avisa que roda em background e posta a tabela de andamento a cada 1 min, com resumo. Motor EMBARCADO, zero instalação. Use ao planejar features, implementar com verificação ou auditar contra a spec. Gatilhos - especificar feature, nova feature, implementar, auditar spec, verificar, plano de execução, executar em paralelo, o que não tem teste, lições aprendidas. NÃO use para technical design docs (technical-design-doc-creator) nem decomposição de arquitetura.
license: MIT
metadata:
