...
### US-013 — Formulários com labels explícitos e separação visual
...
#### AC-031 — Todos os inputs de formulário de cadastro possuem label associado
...
- **Nota complementar:** AC-041 valida a estratégia técnica escolhida para associar o label (`htmlFor` + `id` ou aninhamento válido); AC-031 não se repete em AC-041 — são níveis diferentes (requisito vs. verificação técnica).
- **Verificação:** inspeção do DOM renderizado por página; não é necessário alterar comportamento funcional.

#### AC-032 — Separação visual entre formulário e conteúdo abaixo
...
#### AC-033 — Componentes `EmptyState` utilizados em listas vazias
...
#### AC-034 — Mensagem de vazio configurável no componente
...
### US-015 — Destaque visual consistente para ações principais e destrutivas
...
#### AC-035 — Botões de ação primária possuem variante visual definida
...
#### AC-036 — Botões de ação destrutiva possuem variante visual distinta
...
### US-016 — Hierarquia tipográfica e composição visual
...
#### AC-037 — Título principal da página utiliza tamanho e família tipográfica definidos
...
#### AC-038 — Seções/formulário e conteúdo possuem separação visual verificável
...
### US-017 — Consistência de navegação e indicadores de aba ativa
...
#### AC-039 — Aba ativa possui indicador visual e semântico verificável
...
### US-018 — Acessibilidade visual e consistência dos componentes de estado
...
#### AC-040 — Componentes de interface mantêm semântica acessível
...
#### AC-041 — Labels associados usam uma estratégia de identificação válida
...
- **Nota:** AC-031 e AC-041 são complementares: AC-031 exige que o `label` esteja associado; AC-041 valida a estratégia técnica escolhida, incluindo unicidade de `id`, sem rejeitar labels aninhados.
...
- **Componente `Button`: decisão confirmada.** Será criado `src/ui/components/Button.tsx` com variantes `primary` / `destructive` / `neutral`, justificado pelo uso real em pelo menos uma página (AC-035, AC-036).
- **Token `FONT_SIZE_HEADING`: decisão confirmada.** Deve ser definido como `FONT_SIZE_HEADING = "1.5rem"` em `src/ui/tokens/typography.ts`, obrigatório para AC-037.
- **Navegação: decisão confirmada.** Aba ativa com `aria-current="page"` + indicador visual adicional (`borderBottom` com `COLOR_PRIMARY`).
- **Estado vazio: decisão confirmada.** Uso padronizado de `EmptyState` com `message` configurada (AC-033, AC-034).
- Nenhum componente `FormField` será criado nesta feature; separação entre `label` e `input` feita diretamente nas páginas (AC-031, AC-032, AC-041).
...
|---|---|---|
| AC-031 (`label` associado) | `P2` recorrente: inputs sem `label` (`Login`, `Profissionais`, `Clientes`, `Serviços`, `Produtos`) | `LoginPage.tsx` (placeholder sem label); `ClientesPage.tsx` (placeholder sem label) |
| AC-032 (separação visual) | `P2` recorrente: falta separação entre formulário e conteúdo | `ProfissionaisPage.tsx`, `ClientesPage.tsx` (formulário e lista com `marginBottom: 24` apenas no formulário) |
| AC-033 (`EmptyState`) | `P2`: `EmptyState` subutilizado; listas vazias sem mensagem (`ClientesPage`, `ComandasPage`, `ConfigComissoesPage`) | `EmptyState.tsx` existe; `ClientesPage.tsx` usa `ul` vazio sem `EmptyState` |
| AC-034 (mensagem configurável) | `P2`: `RelatorioEstoquePage` tem mensagem mas sem destaque; `EmptyState` aceita `message` prop | `EmptyState.tsx` (`message?: string`) |
| AC-035 (ação primária) | `P3`: botões `Cadastrar` sem destaque visual consistente; todos com mesmo estilo neutro | `ClientesPage.tsx` (`<button type="submit">Cadastrar cliente</button>` sem variante) |
| AC-036 (ação destrutiva) | `P2`: `Desativar`/`Cancelar` com mesmo estilo neutro | `ClientesPage.tsx` (`<button>Desativar</button>`); `ComandasPage.tsx` (`<button>Cancelar</button>`) |
| AC-037 (tipografia) | `P3`: sem variação entre título de página e corpo; `P2`: títulos de cards menores que necessário | `App.tsx` (`fontSize` não diferenciado); `DashboardPage.tsx` (`h2` sem `fontSize` maior) |
| AC-038 (separação de seções) | `P3`: pouca diferenciação entre seções; `P2`: cards subutilizados | `DashboardPage.tsx` (cards utilizados, mas outras páginas sem `Card`) |
| AC-039 (navegação) | `P3`: aba ativa apenas por `fontWeight: bold`; sem `aria-current` | `App.tsx` (`style={{ fontWeight: abaAtiva === a.id ? "bold" : "normal" }}` sem `aria-current`) |
| AC-040 (semântica) | `P2`/`P3`: `Loading`, `ErrorMessage`, `EmptyState` possuem semântica (`AC-018` atendido) | `Loading.tsx`, `ErrorMessage.tsx`, `EmptyState.tsx` |
| AC-041 (associação válida de label) | `P2`: `label` ausente ou sem estratégia explícita nos formulários | `LoginPage.tsx` (sem `label`), `ConfigComissoesPage.tsx` (label aninhado) |
...
- `Button` será criado em `src/ui/components/Button.tsx` como componente reutilizável, com variantes `primary`, `neutral` e `destructive` (AC-035, AC-036). Nenhum mecanismo CSS específico ou `style` inline é obrigatório.
...
- `AC-031`: `label` associado (`htmlFor` ou aninhamento visível) é requisito básico de acessibilidade para inputs.
- `AC-039`: `aria-current="page"` é requisito básico para navegação.
- `AC-040`: semântica (`role`, `aria-live`) dos componentes de estado deve ser preservada, conforme `AC-018` da feature `fundacao-ui`.
- `AC-041`: todo controle associado deve ter `id` único; a estratégia pode usar `htmlFor` correspondente ou label aninhado válido.
...
- `src/ui/tokens/typography.ts`: `FONT_BODY`, `FONT_SIZE_BODY`, `FONT_HEADING`, `FONT_SIZE_HEADING` (`"1.5rem"`, obrigatório para AC-037).
...
|---|---|---|---|
| ASM-011 | A evolução visual é incremental e não exige redesign completo. | confirmada | Os AC são verificáveis por propriedades observáveis no DOM e no código. |
| ASM-012 | Os tokens existentes e `FONT_SIZE_HEADING` são suficientes para os AC desta feature. | confirmada | Os AC usam `COLOR_PRIMARY`, `COLOR_SECONDARY`, `FONT_BODY`, `FONT_HEADING`, `FONT_SIZE_BODY`, `FONT_SIZE_HEADING = "1.5rem"`, `SPACING_SM`, `SPACING_MD` e `SPACING_LG`; nenhum token adicional é obrigatório. |
...
|---|---|---|---|
| Q-012 | A variante visual de botão primário deve ser implementada por componente reutilizável (`Button`) ou por estilos inline nas páginas? | respondida | Decisão: componente reutilizável `Button` (`primary`/`destructive`/`neutral`). Justificado pelo uso real (AC-035, AC-036). |
| Q-013 | A navegação deve receber indicador visual adicional além de `fontWeight: bold` e `aria-current`? Se sim, qual? | respondida | Decisão: `borderBottom` com `COLOR_PRIMARY` (`crimson`) + `aria-current="page"`. |
| Q-014 | A tipografia de título (`h2`) deve ser definida por constante (`FONT_SIZE_HEADING`) ou pode ser definida diretamente no estilo inline? | respondida | Decisão: criar constante `FONT_SIZE_HEADING = "1.5rem"` em `src/ui/tokens/typography.ts`; obrigatório para AC-037. |
