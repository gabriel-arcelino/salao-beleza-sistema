...
## T-023 — Melhorar navegação com indicador visual e semântico [concluida]
- Refs: US-017, AC-039
...
- Notas: o `h1` do shell em `src/App.tsx` **não** usa `FONT_SIZE_HEADING`, por decisão do dono do produto registrada na validação visual. Tokenizá-lo deixava o título do sistema e o título da página com o mesmo tamanho (24px contra 24px), achatando a hierarquia que a US-016 exige; medido no navegador, são 32px contra 24px sem o token. Uma guarda em `tests/ui/refinamento-interface-tipografia.spec.tsx` impede a regressão: o shell não pode fixar tamanho inline, e a página usa o token. `App.tsx` é dono da T-023 e o título do shell não é escopo de AC-037, que trata do título principal **das páginas**.
