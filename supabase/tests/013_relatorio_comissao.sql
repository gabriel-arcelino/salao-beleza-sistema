...
-- Idempotent fixtures: required data (saloon, user, professional, client,
-- service, command, command items, expenses, adjustments, config)
...
insert into public.comanda_itens (id, salon_id, comanda_id, tipo, servico_id, descricao_snapshot, quantidade, preco_unitario, total, profissional_id, comissao_percentual_snapshot, comissao_valor_snapshot, comissao_processada)
values ('33333333-3333-3333-3333-333333333333', '00000000-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', 'SERVICO', 'cccccccc-cccc-cccc-cccc-cccccccccccc', 'Serviço Teste', 1, 50.00, 50.00, 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 20, 10.00, true)
on conflict do nothing;
...
-- Test 2: Competência com dados (Jan 2026)
select ok(
...
select ok(
    ...
    'other professional with no data @spec:AC-006 @spec:AC-007 @spec:AC-008'
);
